...
void QLearner::printStateSpace(){
    for (std::set< std::vector<float> >::iterator i = statespace.begin();
     i != statespace.end(); i++){
      // cout << "state: " << (*i)[0] << ", " << (*i)[1] << endl; 
      cout << "State: "; 
      printState(*i); 
      cout << endl; 
  }
...
void QLearner::printQ(){
  ...
  for (std::set< std::vector<float> >::iterator i = statespace.begin();
    ...
    // print Q value for each state-action pair 
    for(int j = 0; j < Q_s->size(); j++){
      cout << "Q (action " << j << "): " << (*Q_s)[j] << endl;
    }
