...
#include <rl_env/threerooms.hh>

ThreeRooms::ThreeRooms(Random &rand, bool stochastic, bool rewardType, 
...
ThreeRooms::~ThreeRooms() { delete grid; }

const std::vector<float> &ThreeRooms::sensation() const { 
  //cout << "At state " << s[0] << ", " << s[1] << endl;

  return s; 
}

float ThreeRooms::apply(int action) {
  ...
  if (actUsed > -1){
    ...
    std::cerr << "Unreachable point reached inThreeRooms::apply!!!\n";
  }
...
bool ThreeRooms::terminal() const {
...
int ThreeRooms::getNumActions(){
  return 4;
}


const Gridworld *ThreeRooms::create_default_map() {
  int width = 11;
  int height = 5;
...
ThreeRooms::room_action_t ThreeRooms::add_noise(room_action_t action) {
  switch(action) {
  case NORTH:
  case SOUTH:
...
void ThreeRooms::randomize_goal() {
...
void ThreeRooms::getMinMaxFeatures(std::vector<float> *minFeat,
...
void ThreeRooms::getMinMaxReward(float *minR,
                              float *maxR){
  if (negReward){
    *minR = -1.0;
    *maxR = 0.0;    
...
std::vector<experience>ThreeRooms::getSeedings() {
...
experience ThreeRooms::getExp(float s0, float s1, int a){
